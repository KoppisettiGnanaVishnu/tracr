...
export function calculateTrustScore(blocks) {
  ...
  const totalBugs = blocks.reduce(
    (sum, b) => sum + (b.bugs_found || 0),
    0
  );

  const totalReverts = blocks.reduce(
    (sum, b) => sum + (b.reverted || 0),
    0
  );

  const riskPenalty =
    aiBlocks.length > 0
      ? (riskyAiBlocks.length / aiBlocks.length) * 40
      : 0;
  ...
  const score = Math.max(
    0,
    Math.round(100 - (riskPenalty + bugPenalty + revertPenalty))
  );
  ...
  return {
    ...
    breakdown: {
      ...
      ai_bugs: aiBlocks.reduce(
        (sum, b) => sum + (b.bugs_found || 0),
        0
      ),

      ai_reverts: aiBlocks.reduce(
        (sum, b) => sum + (b.reverted || 0),
        0
      ),

      human_bugs: humanBlocks.reduce(
        (sum, b) => sum + (b.bugs_found || 0),
        0
      ),

      human_reverts: humanBlocks.reduce(
        (sum, b) => sum + (b.reverted || 0),
        0
      ),
    },
